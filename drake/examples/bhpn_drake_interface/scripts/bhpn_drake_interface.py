...
import subprocess
import signal
from pr2_joints_for_base_movement_bhpn_drake_connection\
        import Pr2JointsForBaseMovementBhpnDrakeConnection
...
supported_object_types = {
    'drake_table': interface_path + 'objects/drake_table.sdf',
    'drake_soda': interface_path + 'objects/drake_soda.urdf'}
supported_robot_types = {
    'pr2': (
        Pr2JointsForBaseMovementBhpnDrakeConnection,
        'pr2_simulator',
        'PR2_PLAN',
        'ROBOT_STATE'),
    'valkyrie': (
        ValkyrieBhpnDrakeConnection,
        'valkyrie_simulator',
        'VALKYRIE_MANIP_PLAN',
        'EST_ROBOT_STATE')}

fun_colors = {
    'drake_soda1': 'drake_soda_salmon',
    'drake_soda2': 'drake_soda_blue'}
...
class BhpnDrakeInterface:
    ...
    def __init__(
        ...
        # Make sure that the drake simulation can actually be created with the
        # given arguments.
        if not set([robot_type]).issubset(set(supported_robot_types.keys())):
            raise ValueError(
                'This robot is unsupported by the BHPN-Drake Interface.')
        if not set(object_types.values()).issubset(
                set(supported_object_types.keys())):
            raise ValueError(
                'Some of these objects are unsupported by the BHPN-Drake\
                    Interface.')
        ...
        # Create the robot connection and get the simulator that is specific to
        # the type of robot.
        self.robot_connection = supported_robot_types[self.robot_type][0](
            bhpn_robot_conf)
        self.robot_simulator_executable =\
            supported_robot_types[self.robot_type][1]

        # Information about the drake simulation that will be properly
        # initialized before this class is initialized.
        self.drake_robot_conf = None
        self.contact_results = None
        self.plan_status = None
        self.no_plan_status = no_plan_status
        self.plans_finished = 0
        self.plans_sent = 0
        self.generated_description_paths = []

        # Start the lcm subscribers that listen to the drake simulatiuon for
        # information.
        self.lc = lcm.LCM()
        self.lc.subscribe(
            supported_robot_types[self.robot_type][3],
            self.robot_conf_callback)
        self.drake_viewer_draw_subscription = self.lc.subscribe(
            'DRAKE_VIEWER_DRAW', self.object_pose_callback)
        self.contact_results_subscription = self.lc.subscribe(
            'CONTACT_RESULTS', self.contact_results_callback)
        self.plan_status_subscription = self.lc.subscribe(
            'PLAN_STATUS', self.plan_status_callback)
        self.callback_handler = threading.Thread(
            target=self.handle_lcm_subscribers)
        self.callback_handler.daemon = True
        self.callback_handler.start()

        # Start the drake simulation.
        self.create_bdisc()
        self.drake_simulation = subprocess.Popen(
            'cd ' +
            drake_path +
            '; exec ' +
    ...
    def release(self):
        ...
        print 'Terminated the Drake simulation.\
                Return code of the Drake simulation: ', return_code
    ...
    def generate_description_for_object_name(self, object_name, object_type):
        ...
        type_description_path = drake_path + \
            supported_object_types[object_type].\
            replace(object_type, fun_color_replacement_type)
        ...
        # I know that the following is not robost to every path name, but I
        # think it will be fine (hopefully)
        name_description_path = type_description_path.replace(
            object_type, object_name).replace('objects', 'tmp')
        ...
        return supported_object_types[object_type].replace(
            object_type,
            fun_color_replacement_type).replace(
            object_type,
            object_name).replace(
            'objects',
            'tmp')
    ...
    def create_bdisc(self):
        ...
        for joint in self.robot_connection.interpolate_drake_robot_confs([
                self.bhpn_robot_conf, self.bhpn_robot_conf])[0].joint_position:
        ...
        for object_name, object_type in self.object_types.items():
            ...
            if object_name != object_type:
                object_info += self.generate_description_for_object_name(
                    object_name, object_type) + ' '
            else:
                object_info += supported_object_types[object_type] + ' '
            for value in self.convert_to_drake_pose(
                    self.bhpn_object_confs[object_name][object_name][0]):
        ...
        for index in range(num_objects):
            if index != num_objects - 1:
                ...
                simulator_conf_text += objects[index]
        simulator_conf_file = open(
            interface_path_absolute +
            'tmp/simulator_conf.bdisc',
            'w')
    ...
    def encode_drake_robot_plan(self, bhpn_robot_confs, time_spacing):
        drake_confs = self.robot_connection.interpolate_drake_robot_confs(
            bhpn_robot_confs, time_spacing)

        plan = robot_plan_t()

        # This isn't the actual time, but it is necassary to make all plans
        # sent different. This prevents plans from being identical. The
        # reason why two consecutive plans can't be identical is because
        # Drake's robot_plan_t interpolator currently wont notice that a new
        # plan has been sent after it has recieved the first plan.
        plan.utime = 100000 * self.plans_sent
        ...
        for state in drake_confs:
            # Modify the plan so that drake wont stupidly take the long way
            # from 0 to 2 pi for continuous joints.
            for continuous_joint_index in\
                    self.robot_connection.get_drake_continuous_joint_indices():
                if state.joint_position[continuous_joint_index] - \
                        last_joint_values_on_path[continuous_joint_index] >\
                        math.pi:
    ...
    def command_drake_robot_plan(self, plan):
        ...
        previous_plans_finished = self.plans_finished
        self.lc.publish(
            supported_robot_types[self.robot_type][2], plan.encode())
        self.plans_sent += 1
        if not self.no_plan_status:
            # Wait for the plan to finish.
            while previous_plans_finished == self.plans_finished:
                time.sleep(0.1)
    ...
    def is_gripped(self, hand, object_name):
        return object_name in self.robot_connection.get_gripped_objects([
            (object_name, self.object_types[object_name])], self)[
            object_name][self.robot_connection.get_hands_to_end_effectors()[
                hand]]

    # BHPN Primitives that are robot-secific, and supported in the interface.

    def pick(self, start_conf, target_conf, hand, obj):
        return self.robot_connection.pick(
            start_conf, target_conf, hand, obj, self)
    ...
    def place(self, start_conf, target_conf, hand, obj):
        return self.robot_connection.place(
            start_conf, target_conf, hand, obj, self)
    ...
    def robot_conf_callback(self, channel, data):
        ...
        self.bhpn_robot_conf = self.robot_connection.get_bhpn_robot_conf(
            self.drake_robot_conf)

    def object_pose_callback(self, channel, data):
        msg = lcmt_viewer_draw.decode(data)
        for obj in self.bhpn_object_confs:
            self.bhpn_object_confs[obj] = self.convert_to_bhpn_pose(
                msg.position[msg.link_name.index(obj)], msg.quaternion[
                    msg.link_name.index(obj)])
    ...
    def plan_status_callback(self, channel, data):
        status = plan_status_t.decode(data)
        if self.plan_status is not None:
            if self.plan_status.execution_status == 0 and\
                    status.execution_status == 1:
                # If our previous plan was being executed but now it is done.
                self.plans_finished += 1
