...
# simply loads Valkyrie into BHPN and executes a primitive that BHPN could
# execute with the interface. Also, be sure to start drake_visualizer to see
# what's going on!
...
robot_type = 'valkyrie'
bhpn_robot_conf = makeRobot(
    np.array([(-1.0, -3.0, 0.0), (3.0, 3.0, 2.0)])).nominalConf
bhpn_robot_conf = bhpn_robot_conf.set(
    'robotRightArm', [0.3, 1.25, 0, 0.9, 1.571, 0, 0])
bhpn_robot_conf = bhpn_robot_conf.set(
    'robotLeftArm', [0.3, -1.25, 0, -0.785, 1.571, 0, 0])
...
bhpn_object_confs = {
    'drake_soda': {
        'drake_soda': [soda_pose]}, 'drake_table': {
            'drake_table': [table_pose]}}
fixed_objects = {'drake_table'}

# Construct the interface and tell it to not care about the plan status
# (tell it to consider all plans we send it already completed, so it does not
# block).
...
for index in range(6):
    bhpn_robot_conf = bhpn_robot_conf.set(
        'robotRightArm', [
            0.3 - index * 0.095, 1.25, 0, 0.9, 1.571, 0, 0])
...
for index in range(8):
    bhpn_robot_conf = bhpn_robot_conf.set(
        'robotRightArm', [-0.2 - index * 0.1, 1.25, 0, 0.9, 1.571, 0, 0])
    bhpn_motion_plan.append(bhpn_robot_conf)
