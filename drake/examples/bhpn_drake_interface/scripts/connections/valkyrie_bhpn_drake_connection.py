...
class ValkyrieBhpnDrakeConnection(RobotBhpnDrakeConnection):
    # Note: since valkyrie has legs, but BHPN doesn't command legs (BHPN only commands the planar x, y, theta position of the torso), the joint configuration that is sent to drake contains the x, y, theta position of the valkyrie's torso, followed by the joint values of the valkyrie's upper body joints. So the robot_plan_t that will get sent to the valkyrie drake controller will need to be further broken down. The controller can follow the upper body joint values directly, but the x, y, theta values will need to first be passed to a walking plannner in drake to yield the correct lower body joint values for the controller.
    ...
    def get_bhpn_robot_conf(self, drake_robot_conf):
        ...
        mapping = {'robotBase': drake_joints[0:3],
                   'robotTorso': drake_joints[3:4],
                   'robotHead': drake_joints[4:6],
                   'robotRightArm': drake_joints[6:13],
                   'robotRightGripper': [np.sqrt(drake_joints[13:14][0]/100.0)],
                   'robotLeftArm': drake_joints[15:22],
                   'robotLeftGripper': [np.sqrt(drake_joints[22:23][0]/100.0)]}
    ...
    def get_joint_list(self, bhpn_robot_conf):
        return bhpn_robot_conf['robotBase']\
            + bhpn_robot_conf['robotTorso']\
            + bhpn_robot_conf['robotHead']\
            + bhpn_robot_conf['robotRightArm']\
            + [min([0.5, max([0.15, 100*bhpn_robot_conf['robotRightGripper'][0]**2])])] * 2\
            + bhpn_robot_conf['robotLeftArm']\
            + [min([0.5, max([0.15, 100*bhpn_robot_conf['robotLeftGripper'][0]**2])])] * 2
