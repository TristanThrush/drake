...
class Pr2JointsForBaseMovementBhpnDrakeConnection(RobotBhpnDrakeConnection):
    ...
    def get_bhpn_robot_conf(self, drake_robot_conf):
        ...
        mapping = {'pr2Base': drake_joints[0:3],
                   ...
                   'pr2RightGripper': [
                       np.sqrt(drake_joints[13:14][0] / 100.0)],
    ...
    def interpolate_drake_robot_confs(
            self, bhpn_robot_confs, time_spacing=100000):
        # The deal is that bhpn_robot_confs has to have at least 2 confs.
        assert len(bhpn_robot_confs) >= 2
    ...
    def get_joint_list(self, bhpn_robot_conf):
        return bhpn_robot_conf['pr2Base']\
            ...
            + [min([0.5, max([0.15, 100 * bhpn_robot_conf[
                'pr2RightGripper'][0]**2])])] * 4\
            + bhpn_robot_conf['pr2LeftArm']\
            + [min([0.5, max([0.15, 100 * bhpn_robot_conf[
                'pr2LeftGripper'][0]**2])])] * 4
    ...
    def drake_soda_gripped_function(
        ...
        contact_force_magnitude_threshold = 0.7
        ...
        relevant_contact_results = filter(
            lambda contact_info: (
                (contact_info.body1_name == object_name and (
                    contact_info.body2_name in necessary_collisions_combined))
                or (contact_info.body2_name == object_name and
                    (contact_info.body1_name in
                        necessary_collisions_combined))) and
                            np.linalg.norm(np.array(contact_info.contact_force)) >= contact_force_magnitude_threshold,
            contact_results.contact_info)
        relevant_contact_results_names = map(
            lambda contact_info: contact_info.body1_name if
            contact_info.body2_name == object_name else
            contact_info.body2_name,
        ...
        if necessary_collisions_for_l_gripper_grip.issubset(relevant_names):
            gripper_end_effector_to_gripped_objects['l_gripper_palm_link'] += [
                object_name]
        if necessary_collisions_for_r_gripper_grip.issubset(relevant_names):
            gripper_end_effector_to_gripped_objects['r_gripper_palm_link'] += [
                object_name]
    ...
    def get_object_gripped_function(self, object_type):
        if object_type == 'drake_soda':
            ...
            raise ValueError(
                'A method to tell if this object has been gripped has not been\
                        provided')
    ...
    def maintain_pick_conf(self, bhpn_robot_conf, hand):
        ...
        return bhpn_robot_conf.set(
            bhpn_robot_conf.robot.gripperChainNames[hand],
            [min_width_closed])
    ...
    def pick(
        ...
        start_conf_closed = start_conf.set(
            ...
            n_conf_closed[n_conf_closed.robot.gripperChainNames[hand]])

        # Compute and do the path, given those waypoints
        path = rrt.interpolatePath([
            bhpn_drake_interface_obj.get_bhpn_robot_conf(),
            start_conf_open,
            target_conf_open,
            n_conf,
            n_conf_closed,
